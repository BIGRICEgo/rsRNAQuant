...
def consolidate_rRNA_matrix(gse_id, base_results_path, samples, case_samples, control_samples):
    ...
    output_file_path = os.path.join(gse_stat_path, f"{gse_id}_rRNA_count_matrix.txt")

    if not samples:
        print(f"No samples found for GSE: {gse_id}")
        return None

    # Initialize a dictionary to store sequence information and Counts
    consolidated_data = {}
    ...
    for sample_name in samples:
        ...
        try:
            ...
            required_cols = annotation_cols + ['Counts']
            ...
            for index, row in df_sample.iterrows():
                ...
                if sequence not in consolidated_data:
                    ...
                    # Initialize Count for all samples to 0
                    for s in samples:
                        consolidated_data[sequence][s] = 0.0
                
                # Update Count for the current sample
                consolidated_data[sequence][sample_name] = row['Counts']
    ...
    if case_samples:
        final_df['mean_case'] = final_df[case_samples].mean(axis=1).round(5)
        final_df['median_case'] = final_df[case_samples].median(axis=1).round(5)
    else:
    ...
    if control_samples:
        final_df['mean_control'] = final_df[control_samples].mean(axis=1).round(5)
        final_df['median_control'] = final_df[control_samples].median(axis=1).round(5)
    else:
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python CountMatrix_raw.py <GSE_ID> <BASE_RESULTS_PATH> <SAMPLES> <CASE_CONTROL>")
        sys.exit(1)
